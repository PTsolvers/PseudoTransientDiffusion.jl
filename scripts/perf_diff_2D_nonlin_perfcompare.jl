...
@static if USE_PARALLEL
    @static if USE_PARALLEL_INDICES
        macro qHx(ix,iy) esc(:((g_mu/3.0)*_dx*((H[$ix,$iy] + H[$ix+1,$iy  ])*0.5)^3 * ((H[$ix+1,$iy  ]-H[$ix, $iy]) + (B[$ix+1,$iy  ]-B[$ix,$iy])))) end  # @all(qHx) = g_mu/3.0*_dx*@av_xa(H)^3*(@d_xa(H)+@d_xa(B))
        macro qHy(ix,iy) esc(:((g_mu/3.0)*_dy*((H[$ix,$iy] + H[$ix,  $iy+1])*0.5)^3 * ((H[$ix,  $iy+1]-H[$ix, $iy]) + (B[$ix,  $iy+1]-B[$ix,$iy])))) end  # @all(qHy) = g_mu/3.0*_dy*@av_ya(H)^3*(@d_ya(H)+@d_ya(B))

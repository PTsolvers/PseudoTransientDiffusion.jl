...
@parallel function compute_flux!(qHx, qHy, qHz, qHx2, qHy2, qHz2, H, D, τr_dt, dx, dy, dz)
    ...
    @all(qHz)  = (@all(qHz) * τr_dt - D * @d_zi(H) / dz) / (1.0 + τr_dt)
...
@views function diffusion_3D()
    ...
    if do_viz || do_save_viz
        ...
        if me==0 && do_viz
            ...
            savefig("../../figures/diff_3D_lin3_$(nx_g()).png")
        end
    end
    if me==0 && do_save
        !ispath("../../output") && mkdir("../../output")
        open("../../output/out_diff_3D_lin3.txt","a") do io
            println(io, "$(nx_g()) $(ny_g()) $(nz_g()) $(ittot) $(nt)")
        end
    end
    if me==0 && do_save_viz
        !ispath("../../out_visu") && mkdir("../../out_visu")
        matwrite("../../out_visu/diff_3D_lin3.mat", Dict("H_3D"=> Array(H_v), "xc_3D"=> Array(xc), "yc_3D"=> Array(yc), "zc_3D"=> Array(zc)); compress = true)
    end

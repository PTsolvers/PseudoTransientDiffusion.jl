...
@static if USE_PARALLEL_INDICES
    macro qHx(ix,iy) esc(:((g_mu/3.0)*_dx*((H[$ix,$iy] + H[$ix+1,$iy  ])*0.5) * ((H[$ix+1,$iy  ]-H[$ix, $iy]) + (B[$ix+1,$iy  ]-B[$ix,$iy])))) end  # @all(qHx) = g_mu/3.0*_dx*@av_xa(H)^3*(@d_xa(H)+@d_xa(B))
    macro qHy(ix,iy) esc(:((g_mu/3.0)*_dy*((H[$ix,$iy] + H[$ix,  $iy+1])*0.5) * ((H[$ix,  $iy+1]-H[$ix, $iy]) + (B[$ix,  $iy+1]-B[$ix,$iy])))) end  # @all(qHy) = g_mu/3.0*_dy*@av_ya(H)^3*(@d_ya(H)+@d_ya(B))
    macro dτ() esc(:((1.0/(1.0/dt+1.0/((((min(dx^2,dy^2)/g_mu)*3.0)/H[ix+1,iy+1])/4.1))))) end
end
...
@static if USE_PARALLEL
    @static if USE_PARALLEL_INDICES
        ...
        @parallel function compute_flux!(qHx::Data.Array, qHy::Data.Array, H::Data.Array, B::Data.Array, g_mu::Data.Number, _dx::Data.Number, _dy::Data.Number)
            @all(qHx) = g_mu/3.0*_dx*@av_xa(H)^3*(@d_xa(H)+@d_xa(B))
            @all(qHy) = g_mu/3.0*_dy*@av_ya(H)^3*(@d_ya(H)+@d_ya(B))
            return
        end
